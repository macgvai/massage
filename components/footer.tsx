...
export default function Footer({ siteConfig }: { siteConfig: SiteConfig }) {
  const servicesPreview = siteConfig.services.slice(0, 5);
      ...
      <Container className="py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          <div className="space-y-4">
            <h3 className="text-xl font-bold text-emerald-400">Массажный салон</h3>
            <p className="text-gray-300 text-sm leading-relaxed">
              Профессиональные массажные услуги для вашего здоровья и релаксации. Опытный мастер,
              комфортная атмосфера, индивидуальный подход.
            </p>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-emerald-400">Наши услуги</h3>
            <ul className="space-y-2 text-sm">
              {servicesPreview.map((service) => (
                <li key={service.id}>
                  <a href={`#${service.id}`} className="text-gray-300 hover:text-white transition-colors">
                    {service.title}
                  </a>
                </li>
              ))}
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-emerald-400">Контакты</h3>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-emerald-400">Режим работы</h3>
            <div className="space-y-2 text-sm">
              <div className="flex items-center space-x-3">
                <svg className="w-4 h-4 text-emerald-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span className="text-gray-300">{siteConfig.contact.workingHours}</span>
              </div>
